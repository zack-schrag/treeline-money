...
async def test_update_transaction_tags_persists_changes():
    ...
    try:
        # Create repository
        repo = DuckDBRepository(str(test_dir))

        # Create test user
        user_id = uuid4()

        # Initialize database
        await repo.ensure_user_db_initialized(user_id)
        ...
        # Add the transaction
        result = await repo.add_transaction(user_id, transaction)
        assert result.success

        # Update the tags
        new_tags = ["coffee", "food"]
        update_result = await repo.update_transaction_tags(
            user_id, str(transaction.id), new_tags
        )
