                ...
                if len(tagged_tx.tags) > len(transaction.tags):
                    ...
                    if not dry_run:
                        update_result = await self.repository.update_transaction_tags(
                            user_id, str(transaction.id), list(tagged_tx.tags)
                        )
        ...
        try:
            ...
            for account in accounts:
                ...
                transactions_result = await self.repository.get_transactions_by_account(
                    user_id, account.id, order_by="transaction_date DESC"
                )
                if not transactions_result.success:
                    warnings.append(
